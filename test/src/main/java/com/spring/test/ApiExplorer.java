...
public class ApiExplorer {
    public static void main(String[] args) throws IOException {
        ...
        try {
        while(true) {
	        int firstIndex = sb.indexOf("<contentid>");
	        int LastIndex = sb.indexOf("</contentid>") + new String("</contentid>").length();      
	        contentIdList.add(sb.substring(firstIndex, + LastIndex));
	        
	        sb.replace(0, LastIndex, "");
	        indexCnt += 1;
	        
	        System.out.println(contentIdList.get(indexCnt-1));
        }
        }catch(Exception e) {
